import streamlit as st
import pandas as pd
import plotly.express as px
import plotly.graph_objects as go
import io
from datetime import datetime
import PyPDF2
from docx import Document

# --- إعدادات الصفحة ---
st.set_page_config(page_title="المستشار الذكي للحقائب التدريبية", layout="wide", page_icon="🎓")

# --- تنسيق CSS ---
st.markdown("""
<style>
    .main {direction: rtl;}
    h1, h2, h3, p, div, label, li {text-align: right; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;}
    .stAlert {text-align: right; direction: rtl;}
    .metric-card {background: linear-gradient(135deg, #2c3e50 0%, #4ca1af 100%); 
                  color: white; padding: 20px; border-radius: 15px; text-align: center; box-shadow: 0 4px 6px rgba(0,0,0,0.1);}
    .recommendation-box {background-color: #fff3cd; border-right: 5px solid #ffc107; padding: 15px; margin-bottom: 10px; border-radius: 5px; color: #856404;}
    .report-container {background-color: #f8f9fa; padding: 25px; border-radius: 10px; border: 1px solid #ddd; margin-top: 20px;}
</style>
""", unsafe_allow_html=True)

# --- دوال استخراج النصوص ---
def extract_text_from_pdf(file):
    try:
        pdf_reader = PyPDF2.PdfReader(file)
        text = ""
        for page in pdf_reader.pages:
            text += page.extract_text() or ""
        return text
    except Exception as e:
        return ""

def extract_text_from_word(file):
    try:
        doc = Document(file)
        text = "\n".join([para.text for para in doc.paragraphs])
        return text
    except Exception as e:
        return ""

# --- قاعدة المعرفة (الخبير التربوي) ---
# هنا نربط المعيار -> الكلمات المفتاحية -> التوصية التخصصية
@st.cache_data
def get_expert_knowledge():
    return {
        "المجال الأول: الأهداف ونواتج التعلم": [
            {
                "criterion": "وضوح الهدف العام للبرنامج",
                "keywords": ["الهدف العام", "يهدف البرنامج", "الغرض من البرنامج", "الهدف الرئيس"],
                "advice": "قم بصياغة هدف عام يصف النتيجة النهائية للبرنامج بدقة. مثال: 'تنمية مهارات المتدربين في مجال... بما يمكنهم من...'"
            },
            {
                "criterion": "صياغة نواتج التعلم (SMART)",
                "keywords": ["نواتج التعلم", "الأهداف التفصيلية", "يتوقع من المتدرب", "قادر على أن", "الأهداف السلوكية"],
                "advice": "تأكد أن الأهداف تتبع معيار SMART (محددة، قابلة للقياس، واقعية). استخدم أفعالاً سلوكية مثل: يعدد، يحلل، يصمم، بدلاً من: يفهم، يدرك."
            },
            {
                "criterion": "شمولية الأهداف (معرفي/مهاري/وجداني)",
                "keywords": ["المعرفة", "المهارات", "الاتجاهات", "القيم", "السلوكيات", "الجوانب الوجدانية"],
                "advice": "البرنامج يركز على جانب واحد. أضف أهدافاً وجدانية (مثل: تعزيز الوعي) وأهدافاً مهارية (مثل: تطبيق، استخدام) لضمان التعلم الشامل."
            }
        ],
        "المجال الثاني: المحتوى التدريبي": [
            {
                "criterion": "حداثة المراجع والمصادر",
                "keywords": ["المراجع", "المصادر", "قائمة المراجع", "2023", "2024", "2025", "الدراسات الحديثة"],
                "advice": "لم يتم رصد مراجع حديثة. ادعم المحتوى بإحصائيات ودراسات من آخر 3 سنوات لتعزيز المصداقية العلمية."
            },
            {
                "criterion": "تنظيم وتسلسل الوحدات",
                "keywords": ["الوحدة الأولى", "الجلسة التدريبية", "جدول زمني", "خطة البرنامج", "التسلسل"],
                "advice": "أعد هيكلة المحتوى ليبدأ من الأساسيات وصولاً للتطبيقات المعقدة (Scaffolding). تأكد من وجود رابط منطقي بين الوحدات."
            }
        ],
        "المجال الثالث: الأنشطة والأساليب": [
            {
                "criterion": "تنوع أساليب التدريب",
                "keywords": ["ورشة عمل", "عصف ذهني", "تمثيل أدوار", "دراسة حالة", "نقاش جماعي", "تطبيق عملي"],
                "advice": "الحقيبة تعتمد على السرد النظري. أضف أنشطة تفاعلية لكل 45 دقيقة تدريب (مثل: دراسات حالة، لعب أدوار) لكسر الجمود."
            },
            {
                "criterion": "تعليمات الأنشطة",
                "keywords": ["زمن النشاط", "خطوات النشاط", "المطلوب من المتدرب", "آلية التنفيذ"],
                "advice": "حدد بوضوح لكل نشاط: (الزمن، الهدف، الأدوات المطلوبة، وآلية التنفيذ) في دليل المدرب لتسهيل المهمة."
            }
        ],
        "المجال الرابع: أدوات التقييم": [
            {
                "criterion": "أدوات قياس الأثر (قبلي/بعدي)",
                "keywords": ["الاختبار القبلي", "الاختبار البعدي", "قياس الأثر", "Pre-test", "Post-test"],
                "advice": "صمم اختباراً قبلياً وبعدياً متطابقاً لقياس نسبة التحسن في المعرفة. تأكد أن الأسئلة تغطي جميع نواتج التعلم."
            },
            {
                "criterion": "قياس رضا المتدربين",
                "keywords": ["استمارة تقييم", "راي المتدرب", "استبيان", "تقييم البرنامج"],
                "advice": "أرفق نموذجاً لتقييم بيئة التدريب وأداء المدرب، فهو شرط أساسي لضمان الجودة والتطوير المستمر."
            }
        ]
    }

# --- دالة التقييم الذكي ---
def evaluate_content(text, knowledge_base):
    results = []
    text_lower = text.lower()
    
    for domain, items in knowledge_base.items():
        for item in items:
            # البحث عن الكلمات المفتاحية
            matches = [kw for kw in item['keywords'] if kw in text or kw in text_lower]
            score = len(matches)
            
            status = "غير متحقق"
            if score >= 2:
                status = "متحقق"
            elif score == 1:
                status = "متحقق جزئياً"
            
            results.append({
                "المجال": domain,
                "المعيار": item['criterion'],
                "النتيجة": status,
                "الدرجة": 2 if status == "متحقق" else (1 if status == "متحقق جزئياً" else 0),
                "التوصية": item['advice'], # التوصية الذكية
                "الأدلة": ", ".join(matches) if matches else "لا يوجد"
            })
    return results

# --- مولد التقرير السردي (Consultant Bot) ---
def generate_smart_narrative(df, prog_name):
    score = (df['الدرجة'].sum() / (len(df) * 2)) * 100
    missing_count = len(df[df['النتيجة'] == 'غير متحقق'])
    
    report = f"### 📑 التقرير الاستشاري للبرنامج التدريبي: {prog_name}\n\n"
    
    # المقدمة
    if score >= 85:
        report += "بناءً على التحليل الفني، يظهر البرنامج **جاهزية عالية** ومطابقة ممتازة للمعايير التدريبية، مع وجود بعض التحسينات الطفيفة."
    elif score >= 50:
        report += "البرنامج يمتلك **بنية أساسية جيدة**، ولكنه يحتاج إلى تدخلات جوهرية في جوانب التصميم التعليمي والأنشطة ليرقى لمستوى الاحترافية."
    else:
        report += "يحتاج البرنامج إلى **إعادة هيكلة شاملة**، حيث يفتقر للعديد من العناصر الأساسية في بناء الحقائب التدريبية."

    report += "\n\n#### 🚀 أبرز نقاط القوة:\n"
    strengths = df[df['النتيجة'] == 'متحقق']['المعيار'].tolist()
    if strengths:
        for s in strengths[:3]: report += f"- تميز البرنامج في {s}.\n"
    else:
        report += "- لم يتم رصد نقاط قوة واضحة في المستند المرفق.\n"

    report += "\n#### ⚠️ أولويات التطوير العاجلة:\n"
    weaknesses = df[df['النتيجة'] != 'متحقق']
    
    if not weaknesses.empty:
        # تجميع التوصيات حسب المجال لعدم التكرار
        for domain in weaknesses['المجال'].unique():
            domain_issues = weaknesses[weaknesses['المجال'] == domain]
            report += f"\n**في {domain}:**\n"
            for _, row in domain_issues.iterrows():
                report += f"- **{row['المعيار']}:** {row['التوصية']}\n"
    else:
        report += "لا توجد ملاحظات جوهرية، البرنامج مكتمل.\n"
        
    return report

# --- الواجهة الرئيسية ---
st.title("🎓 المستشار الذكي لتقييم الحقائب التدريبية")
st.markdown("---")

# رفع الملف
col1, col2 = st.columns([2, 1])
with col1:
    uploaded_file = st.file_uploader("ارفع ملف الحقيبة (PDF/Word) وسيقوم النظام بتحليله وكتابة تقرير استشاري:", type=['pdf', 'docx', 'doc'])

if uploaded_file:
    # معالجة الملف
    with st.spinner('جاري قراءة الملف واستدعاء المعايير التربوية...'):
        file_text = ""
        if uploaded_file.name.endswith('.pdf'):
            file_text = extract_text_from_pdf(uploaded_file)
        elif uploaded_file.name.endswith('.docx'):
            file_text = extract_text_from_word(uploaded_file)
        
        if len(file_text) < 50:
            st.error("عذراً، الملف يبدو فارغاً أو لا يمكن استخراج النص منه (قد يكون صوراً ممسوحة ضوئياً).")
        else:
            # التقييم
            kb = get_expert_knowledge()
            results_list = evaluate_content(file_text, kb)
            df_res = pd.DataFrame(results_list)
            
            # الحسابات
            total_score = df_res['الدرجة'].sum()
            max_score = len(df_res) * 2
            percentage = (total_score / max_score) * 100
            
            # عرض النتائج
            st.success("✅ تم الانتهاء من التحليل بنجاح!")
            
            # 1. لوحة القيادة
            st.markdown("### 📊 مؤشرات الجودة")
            m1, m2, m3, m4 = st.columns(4)
            m1.metric("نسبة الجودة", f"{percentage:.1f}%")
            m2.metric("نقاط القوة", len(df_res[df_res['النتيجة']=='متحقق']))
            m3.metric("تحتاج تحسين", len(df_res[df_res['النتيجة']=='متحقق جزئياً']))
            m4.metric("نواقص حادة", len(df_res[df_res['النتيجة']=='غير متحقق']))
            
            st.progress(int(percentage))
            
            # 2. التقرير السردي الذكي
            st.markdown("---")
            st.header("📝 التقرير الاستشاري المولد ذكياً")
            
            smart_report = generate_smart_narrative(df_res, uploaded_file.name)
            
            with st.container():
                st.markdown(f"""<div class="report-container">{smart_report}</div>""", unsafe_allow_html=True)
            
            # 3. جدول التفاصيل مع التوصيات
            st.markdown("---")
            st.header("🔍 التفاصيل الفنية والتوصيات")
            
            # عرض تفاعلي للمشاكل فقط
            issues = df_res[df_res['النتيجة'] != 'متحقق']
            if not issues.empty:
                for i, row in issues.iterrows():
                    with st.expander(f"⭕ {row['المجال']} - {row['المعيار']} ({row['النتيجة']})"):
                        st.markdown(f"""
                        <div class="recommendation-box">
                            <strong>💡 توصية الخبير:</strong><br>
                            {row['التوصية']}
                        </div>
                        """, unsafe_allow_html=True)
            else:
                st.info("ما شاء الله، الحقيبة مكتملة ولا توجد ملاحظات سلبية!")

            # 4. التحميل
            st.markdown("---")
            excel_buffer = io.BytesIO()
            with pd.ExcelWriter(excel_buffer, engine='xlsxwriter') as writer:
                df_res.to_excel(writer, sheet_name='تحليل المعايير', index=False)
                # إضافة ورقة التقرير النصي
                workbook = writer.book
                worksheet = workbook.add_worksheet('التقرير الاستشاري')
                worksheet.write(0, 0, smart_report) # كتابة التقرير في خلية واحدة
                
            st.download_button(
                label="📥 تحميل التقرير الكامل (Excel)",
                data=excel_buffer.getvalue(),
                file_name="Expert_Report.xlsx",
                mime="application/vnd.ms-excel"
            )
